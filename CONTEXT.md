...
- **Flattener** — The core module (`internal/flattener`) that transforms nested YAML structures into flat `map[string]string` with dot notation for objects and bracket notation for arrays. Configured via exported fields (`MaxNestingDepth`, `MaxResultSize`, `MaxYAMLSize`). Instantiated with `flattener.New()`.
