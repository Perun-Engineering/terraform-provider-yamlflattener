...
`
	...
	// Write test files
	if err := os.WriteFile(simpleYAMLPath, []byte(simpleYAML), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(complexYAMLPath, []byte(complexYAML), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(alertmanagerYAMLPath, []byte(alertmanagerYAML), 0644); err != nil {
		t.Fatal(err)
	}
...
func testEquivalenceForKeys(keys []string) resource.TestCheckFunc {
