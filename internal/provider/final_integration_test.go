...
func TestFinalIntegration_CrossPlatformFeatures(t *testing.T) {
	...
	err := os.MkdirAll(filepath.Dir(platformPath), 0750)
	if err != nil {
		t.Fatal(err)
	}
...
`,
	...
	err = os.WriteFile(platformPath, []byte(yamlContent), 0600)
	if err != nil {
		t.Fatal(err)
	}
...
// testFinalIntegrationConfigEquivalence creates a Terraform configuration that tests
// equivalence between data source and function outputs
func testFinalIntegrationConfigEquivalence(yamlContent string) string {
	return fmt.Sprintf(`
provider "yamlflattener" {}
...
// testFinalIntegrationCheckPathEquivalence checks that specific paths have the same values
// in both datasource and function outputs
func testFinalIntegrationCheckPathEquivalence(paths []string) resource.TestCheckFunc {
